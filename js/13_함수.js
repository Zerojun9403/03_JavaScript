        ...
        // querySelector -> #을 작성하지 않아도 되는 메서드로 교체
        //  -> getElementById 로 변경함.
        const btn1 = document.getElementById("btn1");

        /* btn1 클릭시 #input1의 값을 읽어와  print1()함수를 호출 */
        ...
        btn1.addEventListener("click",function(){
            ...
            // input1에 값이 입력이 안되었거나, 빈칸만 입력된경우
            if(value.trim().length ==0){
                //console.log("숫자를 입력해주세요");
                //console.log 개발자가 문제를 확인하는 수단
                alert("숫자를 입력해주세요!");
                //소비자가 문제를 확인할수 있도로고 변경ㅣ
                return // 함수종료 + 호출한 곳으로 돌아감!
            }
            console.log("return 이 여기까지 오는지 확인");
    ...
    btn3a.addEventListener("click", function(){
        ...
        console.log(numbers);
        numbers.push(pow(2,4)); // == numbers.push(16); 2의4승값
        // push 나 numbers[index번호] 를 이용해서 값을 추가할떄 
        // 함수로 만든 기능을 활용해서 숫자나 글자를 추가할 수 있다. 
    });

    // 전달 받은 수를 x제곱해서 반환하는 방식
    function pow (num,x){
        let result = 1; // 곱했을때 결과에 영향을 주지 않기 위해서 1 초기화
        for(let i=0; i<x; i++){
            result *= num;
        }
        return result;
    ...
    /* 화살표 함수 */
    // 클래스가 arrow 은 요소를 모두 가져오기(배열 Array 형태로 확인할 것)
    ...
    arrowList[0].addEventListener("click", () => {
        alert("화살표 함수 기본 형태실습");
    });


    /*  매개 변수 1개인 경우() 생략 가능*/
    arrowList[1].addEventListener("click",e =>{ 
        //e이벤트 객체(모든 이벤트 관련 정보가 담겨있는 객체)
        //e.target : 이벤트 가 발생한 요소 
